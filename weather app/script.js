...
const apiKey = "f731454824de3ffcda26615b16f0ca06";
...
function displayCurrentTemperature(data) {
    ...
    scenery.src = localDate(localToday).getHours() < sunriseHour || localDate(localToday).getHours() >= sunsetHour ? "sunrise.jpg" : "sunset.jpg";
    ...
    if (["Rain", "Drizzle", "Clouds"].includes(weatherType)) {
        ...
        weatherTypeDesc.innerHTML = `<i class="fa-solid fa-cloud-sun"></i> Good Weather`;
    } 
...
function displayGlobalTemperature(city, index) {
    fetchData(`${apiWeather}?q=${city}&appid=${apiKey}&units=${units}`)
        ...
        <div class="row align-items-center flex-nowrap justify-content-center p-3">
          ...
          <img src="https://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png" height="50px" class="col-3 weather-icon global-icon" alt=${data.weather[0].description}>
  
          <p class="text-end col">
            <strong><span class="temps global-temps">${Math.round(data.main.temp)}</span>&deg; 
            
            <span class="fahrenheit">F</span></strong> <br>
  
            <small class="global-description">${data.weather[0].description}</small>
          </p>
          <button class="btn btn-outline-danger remove-btn col-auto"> <i class="fa-solid fa-remove"></i> </button>
        </div>
        `;
...
addToFavBtn.addEventListener("click", addCityToFavorites);
